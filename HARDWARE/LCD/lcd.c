...
void LCD_Init(void)
{ 	 
	RCC->AHB1ENR|=0XF<<3;    	//'??PD,PE,PF,PG???  
	RCC->AHB1ENR|=1<<1;     	//'??PB???  
	RCC->AHB3ENR|=1<<0;     	//'??FSMC???  
	...
	FSMC_Bank1->BTCR[6]|=1<<14;		//???'?ò???????
	FSMC_Bank1->BTCR[6]|=1<<4; 		//????????????16bit 	    
	//????BTR?J???	
	//????????J??? 							    
	FSMC_Bank1->BTCR[7]|=0<<28;		//g?A 	 						  	 
	FSMC_Bank1->BTCR[7]|=0XF<<0; 	//??????????(ADDSET)?15??HCLK 1/168M=6ns*15=90ns	
	//??????????IC?K??????????????????,?????????????????
	FSMC_Bank1->BTCR[7]|=60<<8;  	//??????????(DATAST)?60??HCLK	=6*60=360ns
	//???????J???  
	FSMC_Bank1E->BWTR[6]|=0<<28; 	//g?A 	 							    
	FSMC_Bank1E->BWTR[6]|=9<<0;		//??????????(ADDSET)?9??HCLK=54ns
 	//9??HCLK??HCLK=168M??,?????????IC????z??????????????50ns??  	 
	FSMC_Bank1E->BWTR[6]|=8<<8; 	//??????????(DATAST)?6ns*9??HCLK=54ns
	//'??BANK1,????4
	FSMC_Bank1->BTCR[6]|=1<<0;		//'??BANK1??????1	    
	...
	else if(lcddev.id==0X1963)
	{
		...
		LCD_WR_REG(0xB0);		//????LCDg?
		LCD_WR_DATA(0x20);		//24?g?
		LCD_WR_DATA(0x00);		//TFT g? 
...
void LCD_Clear(u16 color)
{
    ...
    totalpoint *= lcddev.height;    //?õ??????

    LCD_SetCursor(0x00, 0x0000);    //???ù?????
    LCD_WriteRAM_Prepare();         //??'???GRAM
...
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//?õ?????h??????????????????????	
 	num=num-' ';//?õ?t???????ASCII????????'?g??????-' '??????????????
	for(t=0;t<csize;t++)
	{   
